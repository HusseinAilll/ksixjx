...
@fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.سسسبع"))
async def _(event):
    if ispay2[0] == "yes":
        await fifthon.send_file(event.chat_id, 'banned.txt')


@fifthon.on(events.NewMessage(outgoing=True, pattern=r".سلع"))
async def _(event):
    if ispay2[0] == "yes":
...
async def _(event):
    if ispay2[0] == "yes":
        ...
        @fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة بحث"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
...
•- Me : @S_L_3
    ...
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        ...
                        await fifthon.send_message(event.chat.id, "متاح او مزاد")
            else:
                pass
            trys += 1
